...
export class ImportOptimizer {
   ...
   */
  private analyzeImportUsage(code: string, imports: Map<string, ImportData>): ImportUsageAnalysis {
  ...
  private isDynamicImportCandidate(source: string, code: string): boolean {
    ...
    const largeLibraries = ['lodash', 'moment', 'd3', 'chart.js', 'react-select', 'react-table'];
  ...
  private removeUnusedImports(
    ...
    imports.forEach((importData, source) => {
      ...
      if (
        unused &&
        unused.size === importData.named.size &&
   ...
   * Combine imports from the same source
   */
  private combineImportsFromSameSource(imports: Map<string, ImportData>): Map<string, ImportData> {
   ...
   */
  private groupImportsByType(imports: Map<string, ImportData>): Array<Array<[string, ImportData]>> {
  ...
  private replaceImports(code: string, importStatements: string[]): string {
    ...
    return [beforeImports, importStatements.join('\n'), '', afterImports]
      .filter(Boolean)
      .join('\n');
  }
...
const ${names[0]} = await (async () => {
   ...
   */
  public convertToDynamicImports(code: string, candidates: Set<string>): string {
    let optimized = code;
