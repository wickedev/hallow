...
export class CodeOptimizer {
  ...
  private removeUnusedMethods(content: string, usedMethods: Map<string, Set<string>>): string {
    ...
    usedMethods.forEach((methods, serviceName) => {
      ...
      const serviceClassRegex = new RegExp(`class\\s+${serviceName}Stub\\s*{([^}]*)}`, 's');
  ...
  private eliminateDeadCode(content: string): string {
    ...
    let optimized = content.replace(/return[^;]*;[\s\S]*?(?=\n\s*})/g, match => {
  ...
  private collapseDuplicatePatterns(content: string): string {
    ...
    while ((match = methodRegex.exec(content)) !== null) {
      ...
      if (body.length > 50) {
        // Only consider substantial method bodies
    ...
    methodBodies.forEach((count, body) => {
      if (count > 2) {
        ...
        // Add helper function
        optimized = optimized.replace(/^export\s+class/m, `${helper}\n\nexport class`);
  ...
  private applyTreeShakingOptimizations(content: string): string {
    ...
    optimized = optimized.replace(/export\s+class\s+(\w+)/g, '/*#__PURE__*/ export class $1');

    // Add pure annotations to function exports
    optimized = optimized.replace(/export\s+function\s+(\w+)/g, '/*#__PURE__*/ export function $1');

    // Use named exports instead of default exports for better tree-shaking
    optimized = optimized.replace(/export\s+default\s+class\s+(\w+)/g, 'export class $1');
  ...
  private ensureESModules(content: string): string {
    ...
    let esm = content.replace(/module\.exports\s*=\s*{([^}]+)}/g, (match, exports) => {
